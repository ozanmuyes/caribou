...
/*
 * Modified by Ozan Müyesseroğlu on Jun, 16th 2018.
 *
 * Here is the list of modifications;
 * - GE was removed, generic error classes
 *   are exporting instead. See 'errors'
 *   directory for exported classes.
 * - TODO ...
 *
 * TODO ...
 */
...
const debug = require('debug')('caribou:Container');
...
/**
 * Container container instance is used to register and fetch dependencies without
 ...
 * as `Container.bind`, `Container.make` etc directly.
 *
 * @class Container
 */
class Container {
  ...
  _getAutoloadedNamespace(namespace) {
    ...
    return _.find(autoloadedKeys, registeredNamespace => namespace.startsWith(`${registeredNamespace}/`));
  }
   ...
   * provider to be extended via Container container.
   *
   * @method _hasManager
   * @private
   *
  ...
  _resolveAutoloadedPath(namespace) {
    ...
    const result = requireStack(namespace.replace(
      autoloadedNamespace,
      this._autoloads[autoloadedNamespace] // eslint-disable-line comma-dangle
    ));
    if (!result) {
      return result;
    }

    const hooks = (result.ContainerHooks || result.iocHooks);
    if (!_.isArray(hooks)) {
      return result;
    }
  ...
  _makeInstanceOf(Item) {
    ...
    const injections = (Item.inject || []).map(injection => this.make(injection));
   ...
   */
  _require(namespace) { // eslint-disable-line class-methods-use-this
    // FIXME 'class-methods-use-this'
   ...
   * @example
   * ```
   * Container.alias('Adonis/Src/View', 'View')
   * ```
   */
   ...
   * @example
   * ```
   * Container.autoload(path.join(__dirname, './app'), 'App')
   * ```
   */
  ...
  /**
   * Binds a namespace to the Container container as a binding. Given
  ...
  bind(namespace, closure) {
    if (typeof (closure) !== 'function') {
      throw InvalidArgumentException.invalidParameter('Container.bind expects 2nd parameter to be a closure', closure);
    }
  ...
  singleton(namespace, closure) {
    if (typeof (closure) !== 'function') {
      throw InvalidArgumentException.invalidParameter('Container.singleton expects 2nd parameter to be a closure', closure);
    }

    debug('binding %s namespace as singleton to the container', namespace);
   ...
   * to tell Container container that binding can be extended by the
   * outside world using `Container.extend` method.
   *
   ...
   * @example
   * ```
   * Container.extend('App/Foo', 'my-driver', function (app) {
   *   const Config = app.use('Adonis/Src/Config')
  ...
  manager(namespace, bindingInterface) {
    if (typeof (bindingInterface.extend) !== 'function') {
      ...
      throw InvalidArgumentException.invoke(message, 500, 'E_INVALID_IOC_MANAGER');
    }
   ...
   * @example
   * ```
   * Container.extend('Adonis/Src/Session', 'mongo', () => {
  ...
  executeExtendCalls() {
    _.remove(this._extendCalls, ([namespace, key, closure, ...options]) => {
      if (!this._hasManager(namespace)) {
        ...
        throw InvalidArgumentException.invoke(message, 500, 'E_CANNOT_EXTEND_BINDING');
      }

      if (typeof (closure) !== 'function') {
        throw InvalidArgumentException.invalidParameter('Container.extend expects 3rd parameter to be a closure', closure);
      }
   ...
   * @example
   * ```
   * Container.fake('Adonis/Src/Lucid', function () {
   *   return FakeModel
   * })
   *
   * // Restore after testing
   * Container.restore('Adonis/Src/Lucid')
   * ```
   */
  fake(namespace, closure) {
    if (typeof (closure) !== 'function') {
      throw InvalidArgumentException.invalidParameter('Container.fake expects 2nd parameter to be a closure', closure);
    }
   ...
   * @example
   * ```
   * Container.singletonFake('Adonis/Src/Lucid', function () {
  ...
  singletonFake(namespace, closure) {
    if (typeof (closure) !== 'function') {
      throw InvalidArgumentException.invalidParameter('Container.singletonFake expects 2nd parameter to be a closure', closure);
    }
   ...
   * ```
   * Container.restore('Adonis/Src/Lucid')
   * Container.restore('Adonis/Src/Lucid', 'Adonis/Src/Config')
   * Container.restore() // restore all
   * ```
   */
  restore(...namespaces) {
    const processedNamespaces = (namespaces[0] instanceof Array ? namespaces[0] : namespaces);
    if (!processedNamespaces.length) {
      debug('restoring all fakes');
      this._fakes.clear();
    }
    debug('restoring %d fakes', processedNamespaces);
    processedNamespaces.forEach(namespace => this._fakes.delete(namespace));
  }
   ...
   * @example
   * ```
   *  Container.use('View') // via alias
   *  Container.use('Adonis/Src/View') // via complete namespace
   *  Container.use('App/Http/Controllers/UsersController') // autoloaded namespace
   *  Container.use('lodash') // node module
   * ```
   */
  use(namespace) {
   ...
   * registered via `Container.bind` are themselves
   ...
   * }
   *
   * const fooInstance = Container.make(Foo)
   * ```
   */
  make(namespace) {
   ...
   * @example
   * ```
   * Container.makeFunc('App/Http/Controllers/UsersController.index')
   * // returns
  ...
  makeFunc(pattern) {
    ...
    if (!namespace || !method || rest.length) {
      const message = `Container.makeFunc expects a string in module.method format instead received ${pattern}`;
      throw InvalidArgumentException.invoke(message, 500, 'E_INVALID_MAKE_STRING');
    }
    ...
    if (!instance[method]) {
      throw RuntimeException.invoke(`Method ${method} missing on ${normalizedNamespace}`, 500, 'E_UNDEFINED_METHOD');
    }
